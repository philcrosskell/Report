...
# Aliases used in competitor sections
TEAL       = MINT
RED        = CORAL
...
def generate_competitor_pdf(data):
    ...
    # Brand block — yellow pill bar + BEAL wordmark at 17pt
    ...
    # Cover body — business name at 52pt bold
    y = 6 + 108 + 90
    from reportlab.lib.utils import simpleSplit as _ss
    name_lines = _ss(biz_name, 'Helvetica-Bold', 52, CW)
    ...
    # Donut score cards — show competitors analysed + market stat
    n_comp = len(profiles)
    CARD_W = (CW - 42) / 4
    OR_r, IR_r = 33, 23
    ch = OR_r*2 + 38
    ...
    card_defs = [
        (n_comp,  None, 'COMPETITORS'),
        (None,    None, 'MARKET'),
        (None,    None, 'REPORT TYPE'),
        (None,    None, 'DATE'),
    ]
    labels_vals = [
        (str(n_comp),                  'COMPETITORS ANALYSED'),
        ((market[:12] if market else '—'), 'MARKET'),
        ('INTEL',                      'REPORT TYPE'),
        (date_str,                     'PREPARED'),
    ]

    # Draw 4 stat cards — first uses donut, rest use text display
    for i, (val, lbl) in enumerate(labels_vals):
        cx_c = L + i*(CARD_W+14)
        rect(cv, cx_c, y, CARD_W, 5, fill=AMBER)
        rect(cv, cx_c, y+5, CARD_W, ch, fill=LIGHT_BG, stroke=BORDER)
        rcx = cx_c + CARD_W/2
        if i == 0:
            # Donut for competitor count — use fraction of some sensible max (e.g. 10)
            ...
            # Text-based display for other stats
            short_val = val[:8] if len(val) > 8 else val
            txt(cv, rcx, y+5+OR_r+8+14, short_val, bold=True, sz=14 if len(short_val)<=6 else 10, col=AMBER_D, align='center')
        txt(cv, rcx, y+5+OR_r*2+26, lbl, bold=True, sz=7.5, col=LABEL, align='center')
    card_h = OR_r*2 + 5 + 38
    y += card_h + 18

    # Stats strip — all using date_str (never raw ISO)
    stats = [
    ...
    if findings:
        ...
        y = 90
        txt(cv, L, y, f'{n_comp} businesses analysed in this market.', sz=9.5, col=MUTED)
        y += 18
        for f in findings:
            num    = f.get('number', '')
            title  = f.get('title', '')
    ...
    if table_stakes or white_space or noise:
        ...
        if noise:
            ...
            for i2, n in enumerate(noise):
                txt(cv, L+10, y+14+i2*22, '✗', bold=True, sz=10, col=CORAL)
