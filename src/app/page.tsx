  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      ...
      <div className="flex gap-6 px-1 mb-2 text-[11px]" style={{ color: 'var(--t3)' }}>
        <span className="flex items-center gap-1.5"><span style={{ color: 'var(--green)', fontWeight: 700 }}></span> Pass  ÃÂ¢ÃÂÃÂ  in good shape</span>
        <span className="flex items-center gap-1.5"><span style={{ color: 'var(--red)', fontWeight: 700 }}>Fail</span> Fail  ÃÂ¢ÃÂÃÂ  needs fixing</span>
        <span className="flex items-center gap-1.5"><span style={{ color: 'var(--accent)', fontWeight: 700 }}>Fail</span> Warning  ÃÂ¢ÃÂÃÂ  could be better</span>
        <span className="flex items-center gap-1.5"><span style={{ fontWeight: 700 }}>-</span> Unknown  ÃÂ¢ÃÂÃÂ  not publicly visible</span>
      </div>
...
function GbpAuditPage({ onSave }: { onSave: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (json.data.notFound) { setError('Business not found on Google  ÃÂ¢ÃÂÃÂ  check the name and suburb'); return }
  ...
  return (
    <>
      <TopBar title="GBP Audit" sub="Audit any Google Business Profile  ÃÂ¢ÃÂÃÂ  scored and pitch-ready" />
      <div className="flex-1 overflow-y-auto p-6">
        <Card>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {r.scraped && !r.scraped.error
                ...
                : <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-yellow-400/10 text-yellow-400"> Estimated  ÃÂ¢ÃÂÃÂ  page not reachable</span>
              }
            </div>
      ...
      {tab === 'aeo' && (
          <div className="flex flex-col gap-4">
            {r.aeoScore ? (() => {
                  ...
                  <div className="rounded-xl p-4 border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <div className="text-[11px] mt-0.5" style={{ color: 'var(--t3)' }}>Real questions + answers ÃÂ¢ÃÂÃÂ signals Google rich results and AI citation readiness</div>
                      </div>
              ...
              <div className="rounded-xl p-4 border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
                <div className="text-[13px]" style={{ color: 'var(--t3)' }}>AEO score not available ÃÂ¢ÃÂÃÂ re-run the audit to generate AEO data.</div>
              </div>
            )}
          </div>
        )}
...
//  SEO Tab  ÃÂ¢ÃÂÃÂ  Seobility style 
        ...
        {loading && (
          <Card>
            <div className="flex flex-col items-center py-8 gap-4">
              ...
              <div className="text-[12px]" style={{ color: 'var(--t3)' }}>This takes 15-25 seconds  ÃÂ¢ÃÂÃÂ  the AI is researching every player in your market</div>
            </div>
          </Card>
        )}
        ...
        {result && (
          <>
            ...
            <div className="flex items-center gap-3 mb-4 p-4 rounded-xl border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
              ...
              <div className="flex-1">
                <div className="text-[13px] font-semibold">{result.businessName}  ÃÂ¢ÃÂÃÂ  Competitor Analysis Report</div>
    ...
    <div>
          ...
          <table className="w-full text-[13px]" style={{ minWidth: 600 }}>
            ...
            <tbody>{r.profiles.map(p => (
              <tr key={p.url} className="hover:bg-[var(--bg3)] transition-colors">
                ...
                <TD>{p.seoScore != null ? <div className="flex items-center gap-2"><span className="font-bold text-[13px]" style={{ color: scSeo(p.seoScore ?? 0) }}>{p.seoScore}</span><Bar pct={Math.round(((p.seoScore ?? 0)/62)*100)} /></div> : <span style={{ color: 'var(--t3)' }}>ÃÂ¢ÃÂÃÂ</span>}</TD>
                <TD>{p.positioning}</TD>
      ...
      {r.profiles.some(p => p.seoScore != null) && (
        <Card>
          ...
          <div className="text-[12px] mb-4" style={{ color: 'var(--t3)' }}>Deterministic technical SEO scores ÃÂ¢ÃÂÃÂ same checks applied to every URL (title, meta, H1, content, HTTPS, schema, speed and more)</div>
    ...
    return (
      <>
        <div className="px-6 py-4 border-b flex items-center gap-3" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          ...
          <div className="flex-1">
            <div className="text-base font-semibold">{viewingGbp.businessName}  ÃÂ¢ÃÂÃÂ  GBP Audit</div>
    ...
    return (
      <>
        <div className="px-6 py-4 border-b flex items-center gap-3" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          ...
          <div className="flex-1"><div className="text-base font-semibold">{viewingComp.businessName}  ÃÂ¢ÃÂÃÂ  Competitor Analysis</div><div className="text-[12px]" style={{ color: 'var(--t3)' }}>{viewingComp.date}</div></div>
                    ...
                    <tbody>{[...gbpAudits].map(a => {
                      ...
                      return (
                        <tr key={a.id} className="hover:bg-[var(--bg3)] transition-colors">
                          ...
                          <TD>{a.data.rating ? ` ${a.data.rating}` : ' ÃÂ¢ÃÂÃÂ '}</TD>
                          <TD>{a.data.reviewCount ?? ' ÃÂ¢ÃÂÃÂ '}</TD>
