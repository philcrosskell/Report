...
function Tag({ color, children }: { color: string; children: React.ReactNode }) {
  ...
  return <span className={`inline-flex items-center text-[15px] font-medium px-2 py-0.5 rounded ${m[color] ?? m.purple}`}>{children}</span>
}
function Btn({ children, onClick, primary = false, danger = false, sm = false, disabled = false, cls = '' }: { children: React.ReactNode; onClick?: () => void; primary?: boolean; danger?: boolean; sm?: boolean; disabled?: boolean; cls?: string }) {
  const size = sm ? 'px-2.5 py-1 text-[15px]' : 'px-3.5 py-2 text-[15px]'
...
function CTitle({ children }: { children: React.ReactNode }) {
  return <div className="text-[15px] font-semibold mb-3 tracking-wide" style={{ color: 'var(--t2)' }}>{children}</div>
}
function Lbl({ children }: { children: React.ReactNode }) {
  return <label className="block text-[15px] font-medium mb-1.5" style={{ color: 'var(--t2)' }}>{children}</label>
}
function Empty({ icon, title, sub }: { icon: string; title: string; sub: string }) {
  return <div className="text-center py-12 px-6" style={{ color: 'var(--t3)' }}><div className="text-4xl mb-3">{icon}</div><div className="text-[18px] font-semibold mb-1" style={{ color: 'var(--t2)' }}>{title}</div><div className="text-[15px]">{sub}</div></div>
}
function TopBar({ title, sub, children }: { title: string; sub: string; children?: React.ReactNode }) {
  return <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}><div><div className="text-[18px] font-semibold">{title}</div><div className="text-[15px]" style={{ color: 'var(--t3)' }}>{sub}</div></div>{children}</div>
}
function THead({ cols }: { cols: string[] }) {
  return <thead><tr>{cols.map(c => <th key={c} className="text-left text-[15px] font-semibold uppercase tracking-wider" style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', color: 'var(--t3)' }}>{c}</th>)}</tr></thead>
}
function TD({ children, mono = false }: { children: React.ReactNode; mono?: boolean }) {
  return <td className={mono ? 'font-mono text-[15px]' : ''} style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', color: 'var(--t2)' }}>{children}</td>
}
...
function Insight({ color, text }: { color: 'green' | 'red' | 'blue' | 'amber'; text: string }) {
  ...
  return <div className="flex gap-2.5 items-start text-[15px] leading-relaxed"><span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1.5 ${m[color]}`} /><span style={{ color: 'var(--t2)' }}>{text}</span></div>
}
function Spinner() {
  return <div className="w-9 h-9 rounded-full border-2 spin" style={{ borderColor: 'var(--border)', borderTopColor: 'var(--accent)' }} />
}
function SectionDivider({ label }: { label: string }) {
  return <div className="flex items-center gap-3 my-4"><div className="flex-1 h-px" style={{ background: 'var(--border)' }} /><span className="text-[15px] font-semibold uppercase tracking-wider" style={{ color: 'var(--t3)' }}>{label}</span><div className="flex-1 h-px" style={{ background: 'var(--border)' }} /></div>
}
...
function SmartText({ text, className = '', color = 'var(--t2)' }: { text: string; className?: string; color?: string }) {
    ...
    if (items.length >= 2) {
      return (
        <ol className={`flex flex-col gap-1.5 ${className}`} style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {items.map((item, i) => (
            <li key={i} className="flex gap-2.5 items-start text-[15px] leading-relaxed">
              <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[15px] font-bold mt-0.5"
  ...
  if (bulletMatches.length >= 2) {
    const items = text.split(/\n?\s*[â¢\-â]\s+/).map(s => s.trim()).filter(Boolean)
    if (items.length >= 2) {
      return (
  ...
  if (wordCount <= 30) {
    return <p className={`text-[15px] leading-relaxed ${className}`} style={{ color }}>{text}</p>
  }
  ...
  if (cleaned.length <= 1) {
    ...
    return (
      <div className={`flex flex-col gap-2 ${className}`}>
        {chunks.map((c, i) => <p key={i} className="text-[15px] leading-relaxed" style={{ color }}>{c}</p>)}
      </div>
    )
  }
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      {cleaned.map((s, i) => <p key={i} className="text-[15px] leading-relaxed" style={{ color }}>{s}</p>)}
    </div>
  )
}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 py-4 border-b flex items-center gap-3" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
            ...
            <div className="flex-1">
              <div className="text-[18px] font-semibold">{viewingAudit.label || viewingAudit.url}</div>
              <div className="text-[15px] font-mono" style={{ color: 'var(--accent2)' }}>{viewingAudit.url}</div>
            </div>
    ...
    <div className="flex overflow-hidden" style={{ height: '100vh', background: 'var(--bg)' }}>
      <aside className="flex flex-col border-r" style={{ width: 230, minWidth: 230, background: 'var(--bg2)', borderColor: 'var(--border)' }}>
        ...
        <div className="px-4 py-3.5 border-b flex items-center gap-2.5" style={{ borderColor: 'var(--border)' }}>
          ...
          <div>
            <div className="text-[15px] font-bold tracking-wide leading-tight" style={{ color: 'var(--t1)' }}>Audit Machine</div>
            ...
            <div key={section}>
              ...
              {navItems.filter(n => n.section === section).map(item => (
                <button key={item.id} onClick={() => setView(item.id as View)}
                  className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[15px] mb-0.5 transition-all"
                  style={{
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-4 gap-3 mb-5">
          {[['Projects', projects.length, 'var(--accent2)'], ['Pages Audited', audits.length, 'var(--t1)'], ['Avg SEO', avg(seoA) ?? 'â', 'var(--green)'], ['Avg LP Score', avg(lpA) ?? 'â', 'var(--amber)']].map(([l, v, c]) => (
            <div key={String(l)} className="rounded-xl p-4 border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
              <div className="text-[15px] font-semibold uppercase tracking-widest mb-2" style={{ color: 'var(--t3)' }}>{l}</div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {showForm && (
          <Card>
            ...
            <div className="text-[15px] font-semibold uppercase tracking-widest border-b pb-2 mb-3" style={{ color: 'var(--t3)', borderColor: 'var(--border)' }}>Competitors (optional)</div>
            {comps.map((c, i) => (
            ...
            {projects.map(p => {
              ...
              return (
                <div key={p.id} className="rounded-xl p-[18px] border hover:border-yellow-400/30 transition-colors" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
                  ...
                  <div className="font-mono text-[15px] mb-3" style={{ color: 'var(--accent2)' }}>{p.url}</div>
                  <div className="flex gap-4 mb-3">
                    {[['Pages', pa.length, 'var(--t1)'], ['SEO Avg', avgS ?? 'â', 'var(--accent2)'], ['LP Avg', avgL ?? 'â', 'var(--amber)'], ['Comps', p.competitors.length, 'var(--t1)']].map(([l, v, c]) => (
                      <div key={String(l)}><div className="text-[10px]" style={{ color: 'var(--t3)' }}>{l}</div><div className="text-[15px] font-semibold" style={{ color: String(c) }}>{String(v)}</div></div>
                    ))}
                  </div>
                  {p.competitors.length > 0 && (
                    <div className="mb-3 pb-3 border-b" style={{ borderColor: 'var(--border)' }}>
                      {p.competitors.map(c => (
                        <div key={c.name} className="flex gap-1.5 text-[15px] mb-1" style={{ color: 'var(--t2)' }}>
...
function AuditPage({ projects, weights, onRefresh }: { projects: Project[]; weights: LpWeights; onRefresh: () => void }) {
  ...
  const TABS = [{ id: 'gap', label: 'â¡ Gap Analysis' }, { id: 'seo', label: 'SEO Analysis' }, { id: 'lp', label: 'LP Scoring' }, { id: 'fixes', label: 'Priority Fixes' }, { id: 'comp', label: 'Positioning' }, { id: 'sw', label: 'Strengths & Gaps' }, { id: 'recs', label: 'Recommendations' }]
        ...
        {loading && (
          <Card>
            <div className="flex flex-col items-center py-6 gap-4">
              <Spinner />
              <div className="text-[15px]" style={{ color: 'var(--t2)' }}>{STEPS[stepIdx]}...</div>
              <div className="flex flex-col gap-1.5">
                {STEPS.map((s, i) => (
                  <div key={s} className="flex items-center gap-2 text-[15px]" style={{ color: i <= stepIdx ? 'var(--t2)' : 'var(--t3)' }}>
    ...
    <>
      <Card>
        <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="text-[18px] font-semibold">{label || r.overview.pageType}</div>
            ...
            <div className="font-mono text-[15px] mb-2" style={{ color: 'var(--accent2)' }}>{url}</div>
        ...
        <div className="rounded-lg p-3 mb-4 border" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
          <div className="text-[15px] mb-0.5" style={{ color: 'var(--t3)' }}>{url}</div>
          <div className="text-[15px] mb-0.5" style={{ color: 'var(--blue)' }}>{r.overview.title}</div>
          <div className="text-[15px]" style={{ color: 'var(--t2)' }}>{r.overview.description}</div>
        </div>
        {/* Stats row */}
        <div className="grid grid-cols-5 gap-2 mb-4">
          {[['Response', r.overview.responseTime], ['File Size', r.overview.fileSize], ['Words', r.overview.wordCount], ['Int. Links', r.overview.internalLinks], ['Media', r.overview.mediaFiles]].map(([k, v]) => (
            <div key={String(k)} className="rounded-lg p-2.5 border text-center" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
              ...
              <div className="text-[15px] font-semibold">{String(v)}</div>
            </div>
          ))}
        </div>
        {/* Score boxes */}
      ...
      <div className="flex gap-1 border-b mb-5 flex-wrap" style={{ borderColor: 'var(--border)' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => changeTab(t.id)}
            className="px-3 py-2 text-[15px] font-medium border-b-2 -mb-px transition-all whitespace-nowrap"
    ...
    <div>
      <Card>
        <div className="flex items-center gap-6 flex-wrap">
          <div><div className="text-[15px] uppercase tracking-wider mb-1" style={{ color: 'var(--t3)' }}>Current Score</div><div className="text-3xl font-bold" style={{ color: sc(g.beforeScore) }}>{g.beforeScore}<span className="text-[18px] font-normal" style={{ color: 'var(--t3)' }}>/100</span></div><Tag color={stag(g.beforeScore)}>{g.beforeGrade}</Tag></div>
          <div className="text-2xl" style={{ color: 'var(--t3)' }}>â</div>
          <div><div className="text-[15px] uppercase tracking-wider mb-1" style={{ color: 'var(--t3)' }}>After Fixes</div><div className="text-3xl font-bold" style={{ color: 'var(--green)' }}>{g.afterScore}<span className="text-[18px] font-normal" style={{ color: 'var(--t3)' }}>/100</span></div><Tag color="green">{g.afterGrade}</Tag></div>
          <div className="rounded-xl px-5 py-3 border" style={{ background: 'rgba(52,211,153,0.08)', borderColor: 'rgba(52,211,153,0.2)' }}><div className="text-[15px] uppercase tracking-wider mb-1" style={{ color: 'var(--green)' }}>Potential Uplift</div><div className="text-2xl font-bold" style={{ color: 'var(--green)' }}>+{diff} pts</div></div>
          <div className="flex-1 min-w-[200px]"><SmartText text={g.executiveSummary} /></div>
        </div>
      </Card>
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CTitle>ð´ Critical Issues</CTitle>
          {g.criticalIssues.map((item, i) => (
            <div key={i} className="mb-4 pb-4 border-b last:border-0 last:mb-0 last:pb-0" style={{ borderColor: 'var(--border)' }}>
              <div className="flex items-start gap-2 mb-1.5">
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-[15px] font-bold flex-shrink-0 mt-0.5" style={{ background: 'rgba(248,113,113,0.15)', color: 'var(--red)' }}>{i + 1}</div>
                <div className="text-[15px] font-semibold">{item.issue}</div>
              </div>
              <div className="pl-7 mb-1"><span className="text-[15px] font-semibold" style={{ color: 'var(--t2)' }}>Impact: </span><SmartText text={item.impact} color="var(--t3)" /></div>
              <div className="pl-7 mb-2"><span className="text-[15px] font-semibold" style={{ color: 'var(--t2)' }}>Fix: </span><SmartText text={item.fix} color="var(--t2)" /></div>
        ...
        <div>
          <Card>
            ...
            {g.quickWins.map((item, i) => (
              <div key={i} className="flex gap-3 mb-3 pb-3 border-b last:border-0 last:mb-0 last:pb-0" style={{ borderColor: 'var(--border)' }}>
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-[15px] font-bold flex-shrink-0 mt-0.5" style={{ background: 'rgba(52,211,153,0.15)', color: 'var(--green)' }}>{i + 1}</div>
                <div><div className="text-[15px] font-semibold mb-0.5">{item.win}</div><SmartText text={item.action} color="var(--t3)" /><div className="mt-1"><Tag color="blue">{item.timeEstimate}</Tag></div></div>
              </div>
            ))}
          </Card>
          ...
          <div className="rounded-xl p-5 border" style={{ background: 'rgba(255,229,0,0.05)', borderColor: 'rgba(255,229,0,0.3)' }}>
            <div className="text-[15px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--accent2)' }}>â Top Recommendation</div>
          ...
          {cats.map(k => (
            <div key={k}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[15px]" style={{ color: 'var(--t2)' }}>{SEO_LABELS[k]}</span>
                <span className="font-mono text-[15px] font-semibold" style={{ color: sc(r.seoCategories[k].score) }}>{r.seoCategories[k].score}%</span>
              </div>
        ...
        return (
          <Card key={k}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-[15px] font-semibold">{SEO_LABELS[k]}</div>
              <div className="flex items-center gap-2">
                <span className="text-[15px]" style={{ color: 'var(--t3)' }}>{passCount}/{cat.checks.length} passed</span>
                <span className="font-mono text-[15px] font-semibold px-2 py-0.5 rounded" style={{ color: sc(cat.score), background: `${sc(cat.score)}15` }}>{cat.score}%</span>
              </div>
            </div>
            {cat.checks.map((c, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-3 py-2.5 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[15px] font-semibold mb-0.5">{c.label}</div>
                    <div className="text-[15px]" style={{ color: 'var(--t3)' }}>{c.detail}</div>
                  </div>
                  {/* Criticality badge */}
    ...
    <div>
      <Card>
        <div className="flex items-center gap-5 mb-5 flex-wrap">
          ...
          <div><div className="text-2xl font-bold">{r.scores.lp}<span className="text-[15px] font-normal" style={{ color: 'var(--t3)' }}>/100</span></div></div>
          <div className="ml-4"><div className="text-[15px] uppercase tracking-wider mb-1" style={{ color: 'var(--t3)' }}>After fixes</div><div className="text-xl font-bold" style={{ color: 'var(--green)' }}>{r.projectedScoreAfterFixes.total}<span className="text-[15px] font-normal" style={{ color: 'var(--t3)' }}>/100 ({r.projectedScoreAfterFixes.grade})</span></div></div>
        </div>
        {cats.map(k => {
          ...
          return (
            <div key={k} className="mb-4 pb-4 border-b last:border-0 last:mb-0 last:pb-0" style={{ borderColor: 'var(--border)' }}>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[15px] font-semibold min-w-[180px]">{LP_LABELS[k]}</span>
                <Bar pct={c.percentage} />
                <span className="font-mono text-[15px] font-semibold min-w-[55px] text-right" style={{ color: sc(c.percentage) }}>{c.score}/{c.maxScore}</span>
              </div>
    ...
    <Card>
      ...
      {r.priorityFixes.map(f => (
        <div key={f.rank} className="flex gap-3.5 py-3.5 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
          <div className="w-7 h-7 rounded-full flex items-center justify-center text-[15px] font-bold flex-shrink-0" style={{ background: 'rgba(255,229,0,0.12)', color: 'var(--accent2)' }}>{f.rank}</div>
          <div className="flex-1">
            <div className="text-[15px] font-semibold mb-1">{f.title}</div>
            <div className="mb-1 pl-0"><span className="text-[15px] font-semibold" style={{ color: 'var(--t2)' }}>Problem: </span><SmartText text={f.problem} color="var(--t3)" /></div>
            <div className="mb-2"><span className="text-[15px] font-semibold" style={{ color: 'var(--t2)' }}>Fix: </span><SmartText text={f.fix} color="var(--t2)" /></div>
    ...
    <div>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <Card>
          ...
          <div className="mb-3"><div className="text-[15px] uppercase tracking-wider mb-1.5" style={{ color: 'var(--t3)' }}>Hook type</div><Tag color="amber">{c.hookType}</Tag></div>
          <SmartText text={c.hookAnalysis} className="mb-3" />
          <div className="text-[15px] uppercase tracking-wider mb-1.5" style={{ color: 'var(--t3)' }}>Positioning strength</div>
        ...
        <Card>
          ...
          {c.buyerAnxieties.map((b, i) => (
            <div key={i} className="flex gap-2.5 py-1.5 border-b last:border-0 items-start" style={{ borderColor: 'var(--border)' }}>
              <span className="text-[15px] mt-0.5 flex-shrink-0" style={{ color: b.addressed ? 'var(--green)' : 'var(--red)' }}>{b.addressed ? 'â' : 'â'}</span>
              <div><div className="text-[15px] font-semibold">{b.anxiety}</div><div className="text-[15px]" style={{ color: 'var(--t3)' }}>{b.note}</div></div>
            </div>
          ))}
        </Card>
      </div>
        ...
        <Card>
          ...
          {c.whiteSpace.map((w, i) => (
            <div key={i} className="mb-3 pb-3 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
              <div className="text-[15px] font-semibold mb-1">{w.opportunity}</div>
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      {([{ icon: 'â', title: 'Strengths', items: r.strengthsWeaknesses.strengths, color: 'green' as const, bg: 'rgba(52,211,153,0.1)', tc: 'var(--green)' }, { icon: 'â', title: 'Weaknesses', items: r.strengthsWeaknesses.weaknesses, color: 'red' as const, bg: 'rgba(248,113,113,0.1)', tc: 'var(--red)' }, { icon: 'â', title: 'Missed Opportunities', items: r.strengthsWeaknesses.missedOpportunities, color: 'blue' as const, bg: 'rgba(96,165,250,0.1)', tc: 'var(--blue)' }]).map(s => (
        <Card key={s.title}>
          <div className="flex items-center gap-2 mb-3"><div className="w-7 h-7 rounded-md flex items-center justify-center" style={{ background: s.bg, color: s.tc }}>{s.icon}</div><div className="text-[15px] font-semibold">{s.title}</div></div>
        ...
        <Card>
          ...
          {mode === 'manual' ? (
            ...
            <div>
              ...
              {selectedProject && (
                <div className="mt-3 p-3 rounded-lg border text-[15px]" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                  <div className="font-semibold mb-1">{selectedProject.name}</div>
                  <div className="font-mono text-[15px] mb-2" style={{ color: 'var(--accent2)' }}>{selectedProject.url}</div>
                  {selectedProject.competitors.length ? selectedProject.competitors.map(c => (
                    <div key={c.name} className="text-[15px]" style={{ color: 'var(--t3)' }}>vs {c.name} ({c.url})</div>
          ...
          <div className="mt-4 flex items-end gap-4 flex-wrap">
            <div>
              ...
              {brandLogo
                ...
                : <div className="h-10 w-24 rounded border flex items-center justify-center text-[15px]" style={{ borderColor: 'var(--border)', color: 'var(--t3)' }}>No logo</div>
              }
                <div className="flex gap-2">
                  <label className="cursor-pointer inline-flex items-center gap-1.5 px-2.5 py-1 text-[15px] font-medium border rounded-lg transition-all bg-[var(--bg3)] border-[var(--border2)] text-[var(--t1)] hover:bg-[var(--bg4)]">
                    â Upload
              ...
              <div className="text-[15px] mt-1" style={{ color: 'var(--t3)' }}>Appears on the report cover. PNG, JPG, or SVG.</div>
            </div>
        ...
        {loading && (
          <Card>
            <div className="flex flex-col items-center py-8 gap-4">
              <Spinner />
              <div className="text-[15px]" style={{ color: 'var(--t2)' }}>Analysing competitive landscape...</div>
              <div className="text-[15px]" style={{ color: 'var(--t3)' }}>This takes 15â25 seconds â the AI is researching every player in your market</div>
            </div>
          </Card>
        )}
        ...
        {result && (
          <>
            ...
            <div className="flex items-center gap-3 mb-4 p-4 rounded-xl border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
              ...
              <div className="flex-1">
                <div className="text-[15px] font-semibold">{result.businessName} â Competitor Intelligence Report</div>
                <div className="text-[15px]" style={{ color: 'var(--t3)' }}>{result.date} Â· {result.profiles.length} businesses analysed</div>
              </div>
              {!saved ? (
                <>
                  <Btn onClick={saveReport} primary>Save Report</Btn>
                  <Btn onClick={exportPDF}>â Export PDF</Btn>
                  <Btn danger onClick={() => setResult(null)}>Discard</Btn>
                </>
              ) : (
                <>
                  <div className="text-[15px]" style={{ color: 'var(--green)' }}>â Saved to Reports</div>
                  <Btn onClick={exportPDF}>â Export PDF</Btn>
                </>
              )}
            </div>
...
// âââ Competitor Intelligence Report View ââââââââââââââââââââââââââââââââââââââ
    ...
    <div>
      ...
      {brandLogo && (
        <div className="flex items-center gap-4 mb-5 p-4 rounded-xl border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          ...
          <div>
            <div className="text-[15px] font-semibold">{r.businessName}</div>
            <div className="text-[15px]" style={{ color: 'var(--t3)' }}>Competitor Intelligence Report Â· {r.date}</div>
          </div>
        </div>
      )}
      {/* Headline findings */}
      <div className="mb-4">
        ...
        <div className="text-[15px] mb-4" style={{ color: 'var(--t3)' }}>Three findings from analysing {r.profiles.length} businesses in this market.</div>
        {r.headlineFindings.map(f => (
          <div key={f.number} className="flex gap-4 mb-4 p-4 rounded-xl border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
            ...
            <div className="flex-1"><div className="text-[15px] font-semibold mb-2">{f.title}</div><SmartText text={f.detail} /></div>
          </div>
        ))}
      </div>
      ...
      <Card>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="w-full text-[15px]" style={{ minWidth: 600 }}>
      ...
      <Card>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="w-full text-[15px]" style={{ minWidth: 500 }}>
      ...
      <Card>
        ...
        <div className="text-[15px] mb-3" style={{ color: 'var(--t3)' }}>What each business claims â and how specifically.</div>
        <div style={{ overflowX: 'auto' }}>
          <table className="text-[15px]" style={{ minWidth: 600, width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th className="text-left text-[15px] font-semibold uppercase tracking-wider" style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', color: 'var(--t3)', minWidth: 140 }}>Claim Type</th>
                {r.profiles.map(p => <th key={p.url} className="text-left text-[15px] font-semibold uppercase tracking-wider" style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', color: p.tier === 'Client' ? 'var(--accent2)' : 'var(--t3)' }}>{p.name}</th>)}
              </tr>
            </thead>
            <tbody>
              {r.claimsMatrix.rows.map((row, i) => (
                <tr key={i} style={{ background: i % 2 === 1 ? 'var(--bg3)' : 'transparent' }}>
                  <td className="font-semibold text-[15px]" style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', color: 'var(--t2)' }}>{row.claimType}</td>
                  {r.profiles.map(p => (
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <Card>
          ...
          <div className="text-[15px] mb-3" style={{ color: 'var(--t3)' }}>Expected by prospects â not differentiating.</div>
          {r.tableStakes.map((t, i) => <div key={i} className="py-2 border-b last:border-0 text-[15px]" style={{ borderColor: 'var(--border)', color: 'var(--t2)' }}>â¢ {t}</div>)}
        </Card>
        <Card>
          <CTitle>White Space â Worth Claiming</CTitle>
          <div className="text-[15px] mb-3" style={{ color: 'var(--t3)' }}>Claims made by 0â1 competitors. Strong differentiation potential.</div>
        ...
        {r.strategicImplications.map(s => (
          <div key={s.number} className="mb-4 p-5 rounded-xl border" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[18px] font-bold flex-shrink-0" style={{ background: 'rgba(255,229,0,0.12)', color: 'var(--accent2)' }}>{s.number}</div>
      ...
      <Card>
        ...
        <div className="text-[15px] mb-4" style={{ color: 'var(--t3)' }}>Actionable changes executable without a full rebrand.</div>
        <div className="grid grid-cols-2 gap-3">
          {r.quickWins.map((w, i) => (
            <div key={i} className="flex gap-3 p-3 rounded-lg border" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
              <div className="w-6 h-6 rounded-full flex items-center justify-center text-[15px] font-bold flex-shrink-0 mt-0.5" style={{ background: w.effort === 'Easy' ? 'rgba(52,211,153,0.2)' : w.effort === 'Medium' ? 'rgba(251,191,36,0.2)' : 'rgba(248,113,113,0.2)', color: w.effort === 'Easy' ? 'var(--green)' : w.effort === 'Medium' ? 'var(--amber)' : 'var(--red)' }}>â</div>
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold mb-1 leading-snug">{w.action}</div>
      ...
      <div className="p-5 rounded-xl border mb-4" style={{ background: 'rgba(255,229,0,0.04)', borderColor: 'rgba(255,229,0,0.15)' }}>
        <div className="text-[15px] font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--accent2)' }}>Summary</div>
        {(() => {
          ...
          return (
            <>
              {intro && <p className="text-[15px] font-semibold leading-relaxed mb-3" style={{ color: 'var(--t1)' }}>{intro}</p>}
              {bullets.length > 0 && (
              ...
              {closing && (
                <div className="mt-3 p-3 rounded-lg border" style={{ background: 'rgba(255,229,0,0.07)', borderColor: 'rgba(255,229,0,0.35)' }}>
                  <div className="text-[15px] font-semibold uppercase tracking-wider mb-1" style={{ color: 'var(--accent2)' }}>â Key Recommendation</div>
                  <div className="text-[15px] font-semibold" style={{ color: 'var(--t1)' }}>{closing}</div>
                </div>
              )}
            </>
          )
    ...
    return (
      <>
        <div className="px-6 py-4 border-b flex items-center gap-3" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          ...
          <div className="flex-1"><div className="text-[18px] font-semibold">{viewingComp.businessName} â Competitor Intelligence</div><div className="text-[15px]" style={{ color: 'var(--t3)' }}>{viewingComp.date}</div></div>
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <Card>
          ...
          <p className="text-[15px] mb-4" style={{ color: 'var(--t3)' }}>Total should equal 100. Current: <strong style={{ color: total === 100 ? 'var(--green)' : 'var(--red)' }}>{total}</strong></p>
          {(Object.keys(w) as (keyof LpWeights)[]).map(k => (
            <div key={k} className="flex items-center gap-3 mb-3">
              <span className="text-[15px] min-w-[200px]" style={{ color: 'var(--t2)' }}>{labels[k]}</span>
              ...
              <span className="font-mono text-[15px] min-w-[28px]" style={{ color: 'var(--accent2)' }}>{w[k]}</span>
            </div>
          ))}
        ...
        <Card>
          ...
          <p className="text-[15px] mb-4 leading-relaxed" style={{ color: 'var(--t3)' }}>Set in <code className="font-mono rounded px-1" style={{ background: 'var(--bg3)' }}>.env.local</code> locally, or <strong>Vercel â Settings â Environment Variables</strong>.</p>
          {[{ key: 'ANTHROPIC_API_KEY', desc: 'Claude API key (recommended)', href: 'https://console.anthropic.com' }, { key: 'OPENAI_API_KEY', desc: 'OpenAI API key (alternative)', href: 'https://platform.openai.com' }, { key: 'AI_PROVIDER', desc: "Set to 'anthropic' or 'openai'" }, { key: 'PAGESPEED_API_KEY', desc: 'Google PageSpeed (optional)', href: 'https://console.cloud.google.com' }].map(v => (
            <div key={v.key} className="flex items-center gap-3 mb-2 flex-wrap">
              <code className="font-mono text-[15px] rounded px-1.5 py-0.5 min-w-[180px]" style={{ background: 'var(--bg4)', color: 'var(--accent2)' }}>{v.key}</code>
              <span className="text-[15px]" style={{ color: 'var(--t3)' }}>{v.desc}</span>
              {'href' in v && <a href={v.href} target="_blank" rel="noreferrer" className="text-[15px] ml-auto" style={{ color: 'var(--accent2)' }}>â Get key</a>}
            </div>
          ))}
        </Card>
      </div>
