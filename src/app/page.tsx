    ...
    <div className="flex-1 overflow-y-auto p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <Card>
          ...
          {d && (() => {
            const gbpChecks = [
              { label: 'Phone number', pass: !!d.phone, warn: false, action: 'Add your phone number → GBP dashboard → Info' },
              { label: 'Website linked', pass: !!d.website, warn: false, action: 'Add your website URL → GBP dashboard → Info' },
              { label: 'Business description', pass: !!d.hasDescription, warn: false, action: 'Write a 750-char description → GBP → Info → From the owner' },
              ...
              { label: 'Hours set (all days)', pass: !!d.allDaysSet, warn: false, action: 'Set hours for every day → GBP → Info → Hours' },
              { label: 'Holiday hours configured', pass: !!d.holidayHoursSet, warn: true, action: 'Add special hours for public holidays → GBP → Special hours' },
              { label: 'Services listed', pass: !!d.servicesListed, warn: true, action: 'Add your services → GBP → Services' },
              { label: 'Appointment link', pass: !!d.appointmentLink, warn: true, action: 'Add a booking link → GBP → Info → Appointment links' },
              ...
              { label: 'Owner responds to reviews', pass: !!d.ownerRespondsToReviews, warn: false, action: 'Reply to every review → GBP → Reviews' },
              { label: 'No unanswered reviews', pass: (d.unansweredReviews ?? 0) === 0, warn: (d.unansweredReviews ?? 0) > 0 && (d.unansweredReviews ?? 0) < 3, action: 'Reply to outstanding reviews → GBP → Reviews' },
              { label: 'Logo uploaded', pass: !!d.hasLogo, warn: false, action: 'Upload a logo → GBP → Photos → Logo' },
              { label: 'Cover photo uploaded', pass: !!d.hasCoverPhoto, warn: false, action: 'Upload a cover photo → GBP → Photos → Cover' },
              ...
              { label: 'Google Posts active', pass: !!d.hasRecentPosts, warn: false, action: 'Publish a Google Post → GBP → Updates (at least monthly)' },
              { label: 'Service area configured', pass: !!d.serviceAreaSet, warn: false, action: 'Set your service area → GBP → Info → Service area' },
              { label: 'Attributes set', pass: !!d.attributesSet, warn: true, action: 'Add relevant attributes → GBP → Info → From the business' },
            ]
            const fixNow = gbpChecks.filter(c => !c.pass && !c.warn)
            const easyWins = gbpChecks.filter(c => !c.pass && c.warn)
            const passing = gbpChecks.filter(c => c.pass)
            return (
              <div>
                {fixNow.length > 0 && <div className="mb-4">
                  <div className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--red)' }}>Fix now</div>
                  {fixNow.map((c, ci) => (
                    <div key={ci} className="flex items-start gap-3 py-2 border-b last:border-0 text-[12px]" style={{ borderColor: 'var(--border)' }}>
                      <span style={{ color: 'var(--red)', fontWeight: 700, minWidth: 36 }}>Fail</span>
                      <div><div style={{ color: 'var(--t1)', fontWeight: 600 }}>{c.label}</div><div style={{ color: 'var(--t3)' }}>{c.action}</div></div>
                    </div>
                  ))}
                </div>}
                {easyWins.length > 0 && <div className="mb-4">
                ...
                {passing.length > 0 && <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {passing.map((c, ci) => (
                      <span key={ci} className="px-2 py-1 rounded text-[11px]" style={{ background: 'rgba(16,185,129,0.1)', color: 'var(--green)', border: '1px solid rgba(16,185,129,0.2)' }}>{c.label}</span>
                    ))}
                  </div>
                </div>}
              </div>
...
function GbpAuditPage({ onSave }: { onSave: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      try {
        const mapsQ = encodeURIComponent(bizName + ' ' + suburb)
